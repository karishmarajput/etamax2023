...
class UserCheckout(APIView):
  ...
  def post(self, request):
    ...
    def check_criteria(user) -> bool:
      ...
      print(criteria)
      if criteria['C'] >= 1 and criteria['T'] >= 1 and criteria['S'] == 1:
        return False
      else:
        return True
    ...
    if user.is_from_fcrit and check_criteria(user):
      return JsonResponse({"detail": "Criteria Not Satisfied! Atleast 1 cultural , 1 technical and only 1 Department seminar ", "success": False}, status=400)
